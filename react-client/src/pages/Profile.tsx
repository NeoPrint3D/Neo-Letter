    ...
    const statContainers = [
        {
            ...
            icon: <CgGames className="text-blue-500" aria-label="Games Played" size={35} />
        },
        {
            title: "Wins",
            value: user.wins,
            icon: <FaCrown className="text-yellow-500" aria-label="Games Won" size={35} />
        },
        {
            title: "Total Points",
    ...
    return (
        <>
            <Helmet>
                ...
                <meta property="og:url" content={`https://neo-letter.com/profile/${user.username}`} />
                        ...
                        <div className="flex justify-center absolute w-full -translate-y-[2.75rem] ">
                            ...
                            >
                                <div className=" rounded-3xl  border-2 border-white/[10%] w-20  ">
                                    <figure>
                                        <img src={user.profilePic} alt={`A picture of ${user.username} at NeoPrint3D`} referrerPolicy="no-referrer" />
                                    </figure>
                                </div>

                            </m.div>
                        </div>
                        <h1 className="text-center text-5xl font-logo mt-14 text-white"> {user.username} </h1>

                        <div className="flex flex-col items-center mt-5 h-full gap-5">
                            {statContainers.map((stat, index) => (<Stat index={index} key={index} title={stat.title} stat={stat.value} icon={stat.icon} />))}
                        </div>
                    </m.div>
                </div>
            </article>
...
function Stat({ stat, icon, title, index }: { stat: string | number, icon: React.ReactElement, title: string, index: number }) {
    return (
        <section className="w-[90%]">
            <m.div
                ...
                transition={{
                    y: {
                        type: "spring", stiffness: 300, damping: 10, delay: index * .25 + 0.7, duration: 1
                    },
                    opacity: {
                        delay: index * .2 + 0.7, duration: .5
                    },
